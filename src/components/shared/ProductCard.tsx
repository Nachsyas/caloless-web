...
// Database Nutrisi Real-Data Berdasarkan Input
const nutritionData: Record<string, any> = {
  "1": { // Asumsi ID 1 adalah Zensum
    ingredients: [
  ...
  "3": { // Asumsi ID 3 adalah Ubi Cheese Chia Seeds
    ingredients: [
      { name: "Keju (30gr)", kcal: 120 },
      { name: "Ubi Ungu (100gr)", kcal: 86 },
      { name: "Chia Seeds (15gr/1 sdm)", kcal: 73 },
...
export function ProductCard({ product }: { product: any }) {
  ...
  const nutrition = nutritionData[product.id];
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-border p-5 flex flex-col shadow-sm hover:shadow-md transition-shadow h-full group relative overflow-hidden">
      ...
      {/* Area Gambar Produk dengan Efek Hover (Poin 1 & 2) */}
      <div className="relative w-full h-56 rounded-xl overflow-hidden bg-secondary mb-4 border border-border">
        {product.image_url ? (
          <Image
        ...
        {/* Overlay Ingredients & Kalori (Muncul saat disentuh/hover) */}
        {nutrition && (
          <div className="absolute inset-0 bg-black/85 text-white p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center backdrop-blur-sm overflow-y-auto">
            ...
            <ul className="space-y-1.5 text-xs flex-1">
              {nutrition.ingredients.map((ing: any, i: number) => (
                <li key={i} className="flex justify-between items-center">
                  ...
                  <span className="font-mono text-primary font-bold">{ing.kcal} kcal</span>
                </li>
              ))}
            </ul>
            <div className="mt-3 pt-2 border-t border-white/20">
              ...
              <p className="text-[10px] text-gray-400 mt-1.5 leading-tight">{nutrition.note}</p>
            </div>
          </div>
        )}
      </div>
